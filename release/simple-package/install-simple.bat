...
sc create "Lucrum-POS-Middleware" binPath= "\"%~dp0pos-middleware.exe\"" start= auto DisplayName= "Lucrum POS Middleware Service" type= own
...
echo ✓ WebSocket Available: ws://localhost:8080
echo ✓ API Key: test-api-key-123
echo.
echo Management:
echo • To stop: run uninstall.bat
echo • To check status: run test-run.bat
